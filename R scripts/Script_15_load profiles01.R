source("read_dt.R")
test1<-read_dt(103433)
#write.csv(test1, file = "id_112695.csv")
library(xts)
###prova 1 --> works WELL!!
...
#ggplot(holo_wends1,aes(hour, power))+ geom_bar(width=0.9,aes(fill=power),stat = "identity")+facet_wrap(~monthname)+scale_fill_gradientn(colours=c("green","orange","red"))
  ...
  #weekdays+weekends
ggplot(holo_ultim0,aes(hour, power,group=1))+ geom_bar(width=0.9,aes(fill=power),stat = "identity")+scale_fill_gradientn(colours=c("green","orange","red"))#+ facet_wrap(~wd_we)#
ggplot(holo_ultim,aes(hour, power,group=1,fill=wd_we))+ geom_bar(width=0.9,aes(fill=power),stat = "identity")+ facet_wrap(~wd_we)#+scale_fill_gradientn(colours=c("green","orange","red"))
...
ggplot(holo_ultim2,aes(hour, power,color=dayname,fill=dayname))+ geom_bar(stat = "identity",position="dodge")+facet_wrap(~dayname)
