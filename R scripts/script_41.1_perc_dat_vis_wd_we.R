...
names(division_id)<-c("idmeter","date","00:00","01:00","02:00","03:00","04:00","05:00","06:00","07:00","08:00","09:00","10:00","11:00","12:00","13:00","14:00","15:00","16:00","17:00","18:00","19:00","20:00","21:00","22:00","23:00")  

rubi_net<-division_id
...
w_days$dayname <- factor(w_days$dayname, levels = c("dilluns","dimarts","dimecres", "dijous", "divendres")
                         ...
                         holo_ultim1$monthname <- factor(holo_ultim$month, levels = c("gen 2014" , "feb 2014" , "març 2014" ,"abr 2014" , "maig 2014" ,"juny 2014" ,"jul 2014" , "ag 2014"  , "set 2014" , "oct 2014" , "nov 2014" , "des 2014","gen 2015" , "feb 2015" , "març 2015" ))
...
##weekdays
ggplot(holo_wdays,aes(hour, power))+ geom_bar(width=0.9,aes(fill=power),stat = "identity")+scale_fill_gradientn(colours=c("green","orange","red"))
...
#weekends
 ggplot(holo_wends,aes(hour, power))+ geom_bar(width=0.9,aes(fill=power),stat = "identity")+scale_fill_gradientn(colours=c("green","orange","red"))
...
multiplot(a,b, cols=2)
                         ...
                         ggplot(holo_ultim0,aes(hour, power,group=1))+ geom_bar(width=0.9,aes(fill=power),stat = "identity")+
                           labs(title = "LOAD PROFILE",x = "Hour",y="Percentage [%]")+scale_fill_gradientn(colours=c("green","orange","red"))
                         
                         ## differentiate wd_we
                         ggplot(holo_ultim,aes(hour, power,group=1))+ geom_bar(width=0.9,aes(fill=power),stat = "identity")+
                           facet_wrap(~wd_we)+
                           labs(title = "LOAD PROFILES",x = "Hour",y="Percentage [%]")+scale_fill_gradientn(colours=c("green","orange","red"))
                         
                         ##differentiate wd_we dodge
                         ggplot(holo_ultim,aes(hour, power,color=wd_we,fill=wd_we))+ geom_bar(stat = "identity",position="dodge")+
                           labs(title = "LOAD PROFILES",x = "Hour",y="Percentage [%]")
                         ...
                         ggplot(holo_ultim1,aes(hour, power,color=wd_we,fill=wd_we))+ geom_bar(stat = "identity",position="dodge")+facet_wrap(~monthname)+
                           labs(title = "Monthly Load Profiles",x = "Hour",y="Percentage [%]")
                         
                         ##differentiate per day of the week
                         ggplot(holo_ultim2,aes(hour, power,fill=dayname))+ geom_bar(width=0.9,stat = "identity")+facet_wrap(~dayname)+
                           labs(title = "Days Load Profiles",x = "Hour",y="Percentage [%]")
