...
mon1<-test1['2014-12-01::2014-12-31']
...
#####PLOTTING BOTHS WEEKDAYS-WEEKENDS--> facets doesn't work well here
library(ggplot2)
ggplot(hourly_data1,aes(dates, power,color=monthname))+ geom_point()+geom_smooth()
ggplot(hourly_data1,aes(dates, power,color=monthname))+ geom_line()+geom_smooth()
ggplot(hourly_data1,aes(dates, power,color=monthname))+ geom_bar(stat = "identity")
#####PLOTTING WEEKDAYS --> facets doesn't work well here
library(ggplot2)
ggplot(w_days,aes(dates, power,color=monthname))+ geom_point()
ggplot(w_days,aes(dates, power))+ geom_point(aes(color=monthname)) + facet_wrap(~dayname)
#####PLOTTING WEEKENDS --> facets doesn't work well here
library(ggplot2)
ggplot(w_ends,aes(dates, power,color=wd_we))+ geom_point()
ggplot(w_ends,aes(dates, power))+ geom_point(aes(color=monthname)) + facet_wrap(~dayname) 

##PLOTTING ALL JOINT In the same graph, all weekdays vs weekends
library(ggplot2)
ggplot(ultim,aes(dates, power,color=wd_we))+ geom_point()+geom_line()+scale_y_continuous(breaks=c(seq(0,10500,by=500)))
ggplot(ultim,aes(dates, power,color=wd_we))+ geom_bar(width=0.9,stat = "identity")
ggplot(ultim,aes(dates, power,color=wd_we))+ geom_point() + facet_wrap(~monthname)#+geom_smooth(aes(group=1))
